...
def linkedin_companies_parser(url):
    for i in range(5):
        try:
            ...
            print '------------------------------'
            print html.fromstring(formatted_response)
            print '------------------------------'

            if datafrom_xpath:
                try:
                    json_formatted_data = json.loads(datafrom_xpath[0])
                    print json.loads(datafrom_xpath[0])
                    print '------------------------------'
...
def readurls():
    #companyurls = ['https://www.linkedin.com/in/zohaibbrohi']
    companyurls = ['https://www.linkedin.com/company/atlas-informatics']
    extracted_data = []
