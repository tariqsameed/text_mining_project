...
g.parse("/home/sameed/PycharmProjects/CrunchBasedPlayGround/SqlConnecter/offices.nt", format="nt")
...
for getId in objlist:
    ...
    qres = g.query("""select ?subject {
        ?subject <http://www.crunchbased.org/offices/object_id> """+input_object_id+"""}""")
    ...
    for index, elem in enumerate(rowIdArray):
        qres = g.query("""select * {
            <http://www.crunchbased.org/offices/"""+elem+"""> ?predicate ?object}""")
